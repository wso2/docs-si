...
To capture change data via the [WSO2 Integrator: SI Tooling](../develop/streaming-integrator-studio-overview.md), define an input [stream](https://siddhi.io/en/v5.1/docs/query-guide/#stream) with the appropriate schema to capture the information you require, and then connect a [source](https://siddhi.io/en/v5.1/docs/query-guide/#source) of the `cdc` type as shown in the example below.
...
3. Start the MySQL server, create the database and the database table you require as follows:
    ...
    4. [Start the WSO2 Integrator: SI Tooling](../develop/streaming-integrator-studio-overview.md/#starting-streaming-integrator-tooling).
    
    5. Download the `cdc-mysql`Siddhi extension for WSO2 Integrator: SI Tooling. For instructions, see [Installing Siddhi Extensions](../develop/installing-siddhi-extensions.md/#installing-an-extension).
...
4. Create and save the following Siddhi application in WSO2 Integrator: SI Tooling.
