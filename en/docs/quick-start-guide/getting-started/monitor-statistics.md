...
To enable the `SweetFactoryApp` Siddhi application to publish statistics to Prometheus, add the `@App:statistics(reporter = 'prometheus')` annotation to it below the `@App:name` annotation as shown below:
    ...
    ```
!!! tip 
    You can update the Siddhi application in WSO2 Integrator: SI Tooling and deploy it again in the WSO2 Integrator: SI server as you did in [Step 5: Update the Siddhi Application](update-the-siddhi-application.md).
