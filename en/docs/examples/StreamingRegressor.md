...
## Prerequisites
1. Download `siddhi-gpl-execution-streamingml-x.x.x.jar` from the following https://maven.wso2.org/nexus/content/repositories/wso2gpl/org/wso2/extension/siddhi/gpl/execution/streamingml/siddhi-gpl-execution-streamingml/ and copy the jar to `<SI_HOME>/lib`.
2. Shutdown the server and copy this jar file to `<SI_HOME>/lib` location. Re-start the server.
3. Save this sample.
...
## Testing the Sample
1. Train the ML model with training data set. The `<SI_HOME>/samples/artifacts/StreamingRegressor/train.csv` file is used to create the ML model. To simulate the events in this CSV file, execute the following steps.
    ...
    8. Click on 'Upload' button under 'CSV file' section and upload the `<SI_HOME>/samples/artifacts/StreamingRegressor/train.csv` file.
...
3. Send events to `PredictionInputStream` using `<SI_HOME>/samples/artifacts/StreamingRegressor/test.csv` file, to see the predicted values per each event in test.csv. CSV simulation can be done as explained above.
