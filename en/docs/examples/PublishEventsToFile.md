...
## Purpose
This application demonstrates how to configure WSO2 Integrator: SI to publish data events into files in JSON format.

## Prerequisites
* Edit the uri '<SI_HOME>/samples/artifacts/PublishEventsToFile/files/sink/{{name}}.txt' by replacing <SI_HOME> with the absolute path of your WSO2SP home directory. You can also change the path for 'file.uri' in the sink, if you want to publish your event file to a different location.
...
## Viewing the Results
Navigate to the path defined by file.uri (<SI_HOME>/samples/artifacts/0038/files/sink),
...
@sink(type='file', @map(type='json'), file.uri='<SI_HOME>/samples/artifacts/PublishEventsToFile/files/sink/{{name}}.txt')
