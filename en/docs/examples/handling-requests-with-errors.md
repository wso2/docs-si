...
3. To deploy the Siddhi file, follow the procedure below:
    ...
    2. In the **Add New Server** section, enter the host, port, user name and the password of your WSO2 Integrator: SI server as shown below.
        ...
        ![Adding a New Server]({{base_path}}/images/handling-requests-with-errors/add-a-new-server.png)
    ...
    3. In the **Siddhi Apps to Deploy** section, select the check boxes for the **MappingErrorTest.siddhi** and **SinkTransportErrorTest.siddhi** applications. In the **Servers** section, select the check box for the server you added. Then click **Deploy**.
        ...
        ![Select Siddhi Application and Server]({{base_path}}/images/handling-requests-with-errors/select-siddhi-app-and-server.png)
...
1. Start the WSO2 Integrator: SI Tooling server by navigating to the `<SI_TOOLING_HOME>/bin` directory and issuing one of the following commands as appropriate, based on your operating system:
...
1. To open the Error Store Explorer, open WSO2 Integrator: SI Tooling, click **Tools** and then click **Error Store Explorer**.
    ...
    ![Error Store Explorer]({{base_path}}/images/handling-requests-with-errors/error-store-explorer.png)
...
3. You can correct the mapping and replay the event. To do this, change `foo` in the event to `name` and click `replay`.
    ...
    ![Replay Error]({{base_path}}/images/handling-requests-with-errors/replay-error.png)
...
2. In the **Siddhi app** field, select **SinkTransportErrorTest** Siddhi application and then click **Fetch**.
    ...
    ![Error Store Explorer]({{base_path}}/images/handling-requests-with-errors/error-store-explorer-with-SinkTransportErrorTest.png)
    
    This indicates that the event was dropped because the end point was not available.
    
2. To view details of the error, click **Detailed Info**. The following is displayed.

    ![Error Entry]({{base_path}}/images/handling-requests-with-errors/error-entry-for-SinkTransportErrorTest.png)
    
    Previously, you deployed the `ReceiveAndCount` Siddhi application. Therefore, you can send the event again by clicking **Replay**.
    
    ![Replay Error]({{base_path}}/images/handling-requests-with-errors/replay-error-SinkTransportErrorTest.png)
