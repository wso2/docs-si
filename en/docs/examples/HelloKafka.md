...
1. The following steps must be executed to enable SI to receive and publish events via the Kafka transport. Since you need to shut down the server to execute these steps, get a copy of these instructions prior to proceeding.
    ...
    2. Convert and copy the Kafka client jars from the `{KafkaHome}/libs` directory to the `<SI_HOME>/lib` directory as follows.
...
TotalStream will have total of the last 3 events in SweetProductionStream. This is calculated as follows; the sum of
