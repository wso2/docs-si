...
You can monitor the total number of events received by the WSO2 Integrator: SI via its sources per given time interval. This is done by configuring WSO2 Integrator: SI to log the received event count for a specified time interval. The purpose of this log is to estimate the events received by the server during the time it is active. The event counter is reset each time the WSO2 Integrator: SI server is restarted.
